...
import com.liz.common.{ConfigUtil, Constants, DateUtils, HBaseUtil, JedisUtil}
import com.liz.bean._
import com.liz.log.Logging
import com.liz.common.HBaseUtil
...
//  val connection = ConnectionFactory.createConnection(configuration)
  ...
  def loadHBaseData(sparkSession: SparkSession,conf: Configuration): DataFrame = {
    ...
    // 列族
    val familyBytes: Array[Byte] = Bytes.toBytes(Constants.DEFAULT_FAMILY)
    scan.addFamily(Bytes.toBytes(Constants.DEFAULT_FAMILY))
    scan.addColumn(familyBytes,Bytes.toBytes("ORDER_ID"))
    scan.addColumn(familyBytes,Bytes.toBytes("CITY_ID"))
    scan.addColumn(familyBytes,Bytes.toBytes("STARTING_LNG"))
    scan.addColumn(familyBytes,Bytes.toBytes("STARTING_LAT"))
    // 转换格式
    val proScan: ClientProtos.Scan = ProtobufUtil.toScan(scan)
    val scanStr: String = Base64.encodeBytes(proScan.toByteArray)
    ...
    val haikouOrderRdd: RDD[HaiKouOrder] = rddResult.mapPartitions(eachPartition => {
      eachPartition.map(eachResult => {
        val value: Result = eachResult._2
        val order_id: String = Bytes.toString(value.getValue(familyBytes, Bytes.toBytes("ORDER_ID")))
        val city_id: String = Bytes.toString(value.getValue(familyBytes, Bytes.toBytes("CITY_ID")))
        val start_lng: String = Bytes.toString(value.getValue(familyBytes, Bytes.toBytes("START_LNG")))
        val start_lat: String = Bytes.toString(value.getValue(familyBytes, Bytes.toBytes("START_LAT")))
        HaiKouOrder(order_id, city_id, start_lng, start_lat)
      })
    })
